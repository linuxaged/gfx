...
{
#if defined( OS_WINDOWS )
	signal->handle = CreateEventA( nullptr, !autoReset, FALSE, nullptr );
#elif defined( OS_HEXAGON )
...
#else
	pthread_mutex_init( &signal->mutex, nullptr );
	pthread_cond_init( &signal->cond, nullptr );
	signal->waitCount = 0;
	signal->autoReset = autoReset;
	signal->signaled = false;
#endif
...
static void ksThread_SetAffinity( int mask )
{
	...
	if ( !SetThreadAffinityMask( thread, mask ) )
	{
		...
		FormatMessageA( FORMAT_MESSAGE_FROM_SYSTEM, nullptr, error, MAKELANGID( LANG_NEUTRAL, SUBLANG_DEFAULT ), buffer, sizeof( buffer ), nullptr );
...
static bool ksThread_Create( ksThread * thread, const char * threadName, ksThreadFunction threadFunction, void * threadData )
{
	...
	DWORD threadID;
	thread->handle = CreateThread( nullptr, stackSize, (LPTHREAD_START_ROUTINE)ThreadFunctionInternal, thread, STACK_SIZE_PARAM_IS_A_RESERVATION, &threadID );
	if ( thread->handle == 0 )
	{
		return false;
	}
...
static void ksThreadPool_Create( ksThreadPool * pool, const int numWorkers )
{
	...
	{
		ksThread_Create( &pool->threads[i], "worker", PoolStartThread, nullptr );
